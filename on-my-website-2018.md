...
::: I intended to have these equations used by the [SimPEG community](/simpeg), but the uptake was ~0, although enthusiasm was notoriously high ([7 likes, wooo!](/twitter-reuse-equations) ha). In the [GeoSci community](/geosci), however, the development of ideas is necessarily more collaborative and for consistency of equations across the entire resource of interconnected pages, it was worth the time to invest in an [equation bank](https://github.com/geoscixyz/em/tree/master/content/equation_bank). This allows us to debate and invest time in notation, which [is important](/on-breakthroughs). The equation bank is a list of equations with correct notation, and expose these to the entire GeoSci not-a-textbook textbook. For example, pulling from the article on [Lenz's Law](/geosci-lenzs-law) we can see that the equations are dynamically `.. include::`-ed - the [restructured text](/rst) equivalent of an import statement.
